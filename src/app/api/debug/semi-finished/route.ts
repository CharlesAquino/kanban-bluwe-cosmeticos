...
export async function GET(request: NextRequest) {
  try {
    ...
    const details = items.map(item => ({
      ...
      percentEnvasado: item.quantityTotal > 0 ? 
        Math.round((Number(item.quantityEnvasado || 0) / Number(item.quantityTotal)) * 100) : 0,
      op: item.op,
      batch: item.batch,
      shouldBeInQuarantine: (Number(item.quantityEnvasado || 0) >= Number(item.quantityTotal)) && Number(item.quantityTotal) > 0,
      createdAt: item.createdAt,
      updatedAt: item.updatedAt
    }))
    
    // Análise específica
    const gelConstrutor = details.find(item => 
    ...
    if (gelConstrutor) {
      ...
      console.log(`   Deveria estar em quarentena: ${gelConstrutor.shouldBeInQuarantine}`)
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        analysis: {
          ...
          readyForQuarantine: details.filter(item => item.shouldBeInQuarantine && item.status !== 'QUARENTENA'),
          inQuarantine: details.filter(item => item.status === 'QUARENTENA'),
          needsPackaging: details.filter(item => item.envasadoKg > 0 && item.envasadoKg < item.totalKg)
        }
      }
    })
